...
impl BlockMeta {
    ...
    pub fn encode_block_meta(block_meta: &[BlockMeta], max_ts: u64, buf: &mut Vec<u8>) {
        ...
        estimated_size += std::mem::size_of::<u32>(); // hash
        estimated_size += std::mem::size_of::<u64>(); // max ts
    ...
    pub fn decode_block_meta(mut buf: impl Buf) -> Result<(Vec<BlockMeta>, u64)> {
        ...
        let checksum = crc32fast::hash(&buf.chunk()[..&buf.remaining() - 4]);
        for _ in 0..num {
            let offset = buf.get_u32() as usize;
            let first_key_len = buf.get_u16() as usize;
            let first_key = buf.copy_to_bytes(first_key_len);
            let fisrt_key_ts = buf.get_u64();
            let last_key_len = buf.get_u16() as usize;
            let last_key = buf.copy_to_bytes(last_key_len);
            let last_key_ts = buf.get_u64();
            block_meta.push(BlockMeta {
                offset,
                first_key: KeyBytes::from_bytes_with_ts(first_key, fisrt_key_ts),
...
impl SsTable {
    ...
    pub fn open(id: usize, block_cache: Option<Arc<BlockCache>>, file: FileObject) -> Result<Self> {
        ...
        let raw_bloom_offset = file.read(
            file_len - std::mem::size_of::<u32>() as u64,
            std::mem::size_of::<u32>() as u64,
        )?;
        let bloom_offset = (&raw_bloom_offset[..]).get_u32() as u64;
        let raw_bloom_data = file.read(
            bloom_offset,
            file_len - bloom_offset - std::mem::size_of::<u32>() as u64,
        )?;
        let bloom = Bloom::decode(&raw_bloom_data)?;

        let raw_meta_offset = file.read(bloom_offset - 4, 4)?;

        let meta_offset = (&raw_meta_offset[..]).get_u32() as u64;
        let raw_block_meta = file.read(meta_offset, bloom_offset - meta_offset - 4)?;

        let (block_meta, max_ts) = BlockMeta::decode_block_meta(raw_block_meta.as_slice())?;
        let sstable = Self {
            file,
    ...
    pub fn read_block(&self, block_idx: usize) -> Result<Arc<Block>> {
        let block_meta = self.block_meta.get(block_idx).context("out of bound")?;
        let block_offset = block_meta.offset;
        let block_offset_end = self
            .block_meta
            .get(block_idx + 1)
            .map_or(self.block_meta_offset, |x| x.offset);
        let block_len = block_offset_end - block_offset - 4;
        let block_raw_data_with_check_sum = self.file.read(
            block_offset as u64,
            (block_offset_end - block_offset) as u64,
        )?;
        let block_raw_data = &block_raw_data_with_check_sum[..block_len];
        let checksum = (&block_raw_data_with_check_sum[block_len..]).get_u32();
        if checksum != crc32fast::hash(block_raw_data) {
