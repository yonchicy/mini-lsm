...
impl LsmStorageInner {
        ...
        if !manifest_path.exists() {
            ...
            if options.enable_wal {
                ...
                for id in memtables.iter() {
                    ...
                    let mem_max_ts = memtable
                        .map
                        .iter()
                        .map(|x| x.key().ts())
                        .max()
                        .unwrap_or_default();
                    max_ts = mem_max_ts.max(max_ts);
            ...
            next_sst_id += 1;
            manifest = m;
            println!("state from recovery");
            state.dump_structure();
        }
    ...
    pub fn get_with_ts(&self, _key: &[u8], read_ts: u64) -> Result<Option<Bytes>> {
        ...
        for immutable_memtable in &snapshot.imm_memtables {
            memtable_iters.push(Box::new(immutable_memtable.scan(
                Bound::Included(KeySlice::from_slice(_key, TS_RANGE_BEGIN)),
                Bound::Included(KeySlice::from_slice(_key, TS_RANGE_END)),
            )))
        }
        ...
        let iter = LsmIterator::new(
            TwoMergeIterator::create(
                TwoMergeIterator::create(memtable_merge_iter, l0_iterator)?,
                MergeIterator::create(levels_concat_iters),
            )?,
            Bound::Unbounded,
            read_ts,
        )?;
            ...
            match record {
                WriteBatchRecord::Put(key, value) => {
                    ...
                    let size = {
                        ...
                        state.memtable.put(KeySlice::from_slice(_key, ts), _value)?;
                ...
                WriteBatchRecord::Del(key) => {
                    let _key = key.as_ref();
                    let state = self.state.read();
                    state.memtable.put(KeySlice::from_slice(_key, ts), b"")?;
                    if state.memtable.approximate_size() > self.options.target_sst_size {
                        let lock = self.state_lock.lock();
                        if state.memtable.approximate_size() > self.options.target_sst_size {
                            drop(state);
    ...
    pub fn write_batch<T: AsRef<[u8]>>(
        ...
        if !self.options.serializable {
            let _ = self.write_batch_inner(_batch)?;
        } else {
            let txn = self.mvcc().new_txn(self.clone(), self.options.serializable);
            for rec in _batch {
                match rec {
    ...
    pub fn put(self: &Arc<Self>, _key: &[u8], _value: &[u8]) -> Result<()> {
        let record: WriteBatchRecord<&[u8]> = WriteBatchRecord::Put(_key, _value);
    ...
    pub fn scan_with_ts(
        ...
        let mut mem_iterator = MergeIterator::create(memtable_iters);
        if let Bound::Excluded(key) = _lower {
            while mem_iterator.is_valid() && mem_iterator.key().key_ref() == key {
                mem_iterator.next()?;
            }
        }
        ...
        for &sst_id in snapshot.l0_sstables.iter() {
            ...
            if Self::range_overlap(
                ...
                let sst_iter = match _lower {
                    ...
                    Bound::Excluded(key) => {
                        let k = KeySlice::from_slice(key, TS_RANGE_BEGIN);
        ...
        let lsm_iterator = LsmIterator::new(lsm_iter_inner, map_bound(_upper), read_ts)?;
